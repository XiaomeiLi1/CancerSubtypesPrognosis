...
#'            "hsa-miR-21", "hsa-miR-155", "hsa-miR-210", "miRNA10"
#' @import Biobase
#' @import stringr
...
#' @param methods A set of methods to be performed in the 4 methods: "hsa-miR-21", "hsa-miR-155", "hsa-miR-210", "miRNA10".
#' @examples 
#' data(TCGA500)
...
#'  \item \strong{hsa-miR-210}: Camps C, Buffa FM, Colella S, Moore J, Sotiriou C, Sheldon H, et al. hsa-miR-210 Is induced by hypoxia and is an independent prognostic factor in breast cancer. Clin Cancer Res. 2008;14(5):1340-1348. Wang J, Zhao J, Shi M, Ding Y, Sun H, Yuan F, et al. Elevated expression of miR-210 predicts poor survival of cancer patients: a systematic review and meta-analysis. PLoS One. 2014;9(2):e89223.
...
CancerPrognosis_miRNAData <- function(data, methods) 
{
  ...
  else
  {
    ...
    ##1. hsa-miR-21
    if ("hsa-miR-21" %in% methods) {
      cat("Processing on hsa-miR-21.... \n")
      single.sig <- "hsa-miR-21"
      #Coefficient from univariate cox regression
      wight <- 1
      index = which(toupper(dannot$Gene.symbol) == toupper(single.sig))
      if(length(index) == 0) {
        single.sig <- c("hsa-miR-21-3p","hsa-miR-21-5p")
      ...
      if (is.na(tmp1)) {
        cat("Failed ... Check your data for hsa-miR-21 signatures \n")
      }
      rest = cbind(rest, "miR-21" = tmp1)
    }
